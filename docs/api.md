...
| `GET /v1/workspaces/:workspace/feeds/:id`                                       | Read one owned feed and its current newest-first screenshots (each item includes `pageUrl` at `/c/<id>/<item>`); requires `files:read`                                                                                          |
