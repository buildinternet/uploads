...
| Branch-staged attachments after promotion                     | **Never deleted by promotion or cron** | Promotion into a PR's attachment prefix is copy-and-keep — the staged original under `gh/<repo>/branch/<branch>/…` stays in place, so any URL ever handed out (including ones agents embedded in PR bodies pre-promotion) keeps serving directly from storage. This is a contract, not an implementation accident: durable references come from never invalidating issued URLs, not from a proxy layer. Staged objects are deleted only by the normal per-workspace retention purge or explicit `files:delete`, like any other object — no carve-out, no age-based cleanup of any kind. **History:** a `promoted-at`+7d reaper (plus a 30-day "abandoned" rule) shipped in #314 and ran live on cron; it was retired in #421 because the `promoted` rule was exactly this forbidden deletion, and the `abandoned` rule could destroy URLs embedded in GitHub issues or in long-open PRs. See #404 for the original contract and #421 for the retirement — read both before reintroducing any time-based staging cleanup. |
...
| Self-serve workspace deletion (`DELETE /v1/workspaces/:name`) | **Soft** by default, 14-day grace      | Session-authed; `selfServe === true` required, and the caller must be either the record creator (`createdByUserId`) or hold org role `owner` (not `admin`) in that workspace's org (#265). Same stamp/grace mechanics as the admin path above via a shared helper (`stampSoftDelete`/`stampRestore` in `apps/api/src/workspace.ts`) — the two can't drift. `POST /v1/workspaces/:name/restore` mirrors the admin restore semantics exactly (409 `not_deleted`, 410 `grace_expired`). Never a hard/force mode; never frees the slug. Deliberately session-only — a `workspace:manage` bearer token cannot delete or restore. No web console UI yet — API only.                                                                                                                                                                                                                                                                                                                                                            |
