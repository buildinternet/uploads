...
5. Apply D1 migrations remotely **before** deploying API code that depends on
   them, then deploy:
   ```bash
...
6. Register the workspace: `pnpm workspace:add default` — with no flags it
   ...
   `--binding` / `--public-base-url`) for a dedicated bucket instead. The
   script first creates the workspace's organization through the deployed
   API, so it needs `ADMIN_TOKEN` in the environment. Pass `--api-url` when
   your API is not at `https://api.uploads.sh`.
