...
- **Still owed after that sync:** the three `templates/*.dc.html` page comps
  (account, landing-page, workspace-settings) are hand-authored and are *not*
  regenerated by a component sync, so they still carry pre-scale sizes and mono
  chrome. There is also no typography/foundations card — the scale is the most
  lookup-worthy thing in the system and nothing in the pane documents it.
