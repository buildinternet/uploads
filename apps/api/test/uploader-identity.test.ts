...
describe("uploaderTags", () => {
  ...
  it("resolves account id → login and caches it", async () => {
    ...
    const tags = await uploaderTags(env, "user-1", githubFetch("octocat"));
    ...
    const again = await uploaderTags(env, "user-1", githubFetch("changed"));
