...
// uploads#754 item 3 (minimal-binding profile): AUTH and API are optional
// here even though wrangler.jsonc declares them unconditionally.
...
// lib/api-proxy.ts); this augmentation only centralizes the documentation on
// the shared `Env` so an audit doesn't have to hunt through multiple files to
// see which bindings a self-hoster may skip. ASSETS is NOT listed here — it
// is a core binding (the site cannot serve without it); see docs/ops.md.
interface Env {
  AUTH?: Fetcher;
  API?: Fetcher;
  /**
