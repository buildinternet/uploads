...
/** Better Auth API key row (hashed secret omitted). */
...
export type CreateApiKeyResult =
  | { ok: true; key: string; record: DeveloperApiKey }
  | { ok: false; message: string };

function asApiKey(value: unknown): DeveloperApiKey | null {
  ...
  return {
    ...
    expiresAt: (row.expiresAt as string | Date | null | undefined) ?? null,
    lastRequest: (row.lastRequest as string | Date | null | undefined) ?? null,
  };
}

/** Parse `/api-key/list` — object `{ apiKeys }` or a bare array. */
export function parseApiKeyList(body: unknown): DeveloperApiKey[] | null {
  const rows = Array.isArray(body)
    ? body
    : body && typeof body === "object" && Array.isArray((body as { apiKeys?: unknown }).apiKeys)
      ? (body as { apiKeys: unknown[] }).apiKeys
      : null;
  if (!rows) return null;
  const out: DeveloperApiKey[] = [];
  for (const row of rows) {
    const parsed = asApiKey(row);
...
/** GET /api/auth/api-key/list — the signed-in user's keys (secret omitted). */
export async function listApiKeys(origin: string): Promise<DeveloperApiKey[] | null> {
  const result = await fetchWithTimeout(`${authOrigin(origin)}/api/auth/api-key/list`, {
    credentials: "include",
    cache: "no-store",
  });
  if (result.kind === "unavailable" || !result.response.ok) return null;
  const body = await result.response.json().catch(() => undefined);
  return parseApiKeyList(body);
}

/** POST /api/auth/api-key/create — secret is returned once. */
export async function createApiKey(origin: string, name: string): Promise<CreateApiKeyResult> {
  try {
    const res = await fetch(`${authOrigin(origin)}/api/auth/api-key/create`, {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
    ...
    const record = asApiKey(body);
    const key = typeof body?.key === "string" ? body.key : "";
    if (!record || !key) return { ok: false, message: "Auth returned a malformed API key." };
    return { ok: true, key, record };
  } catch {
    return { ok: false, message: "Could not create an API key." };
  }
}

/** POST /api/auth/api-key/delete. */
export async function deleteApiKey(origin: string, keyId: string): Promise<boolean> {
  try {
    const res = await fetch(`${authOrigin(origin)}/api/auth/api-key/delete`, {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
