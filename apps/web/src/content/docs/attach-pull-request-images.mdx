...
<div class="pointer">
  Clicking an image in the managed comment opens that PR's feed pager
  (<code>/c/&lt;id&gt;/&lt;item&gt;</code>), not a standalone file page. Syncing the comment
  creates the feed if needed. See
  <a href="/docs/feeds">change feeds <span class="go">→</span></a>.
</div>
...
no longer loses that context. `path` and `state` stay the labels you set;
[experimental AI labels](/docs/agents#experimental-ai-tags) are a separate, server-owned layer.
...
If you rename the branch, running any `uploads` staging or promote command on it — `attach
--branch`, `put`, `attach --pr`, `attach --promote` — reads the rename from the branch's git
